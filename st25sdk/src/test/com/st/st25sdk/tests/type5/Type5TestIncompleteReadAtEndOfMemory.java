...
public class Type5TestIncompleteReadAtEndOfMemory {
    ...
    static public void run(Type5Tag type5Tag) throws STException {
        ...
        // Allocate some bytes and write them at the end of the memory
        byte[] randomData = new byte[DATA_SIZE_IN_BYTES];
        new Random().nextBytes(randomData);

        int memorySizeInBytes = type5Tag.getMemSizeInBytes();
        int startAddress = memorySizeInBytes-DATA_SIZE_IN_BYTES;
        ...
        if (!NFCTagUtils.isReadLooping(type5Tag)) {
            ...
            // Read more blocks than available
